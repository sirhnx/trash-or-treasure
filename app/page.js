...
const API_CONFIG = [
  ...
  { id: "ebay", label: "eBay App ID", icon: "🛒", required: false, placeholder: "YourApp-XXXX-XXXX-XXXX-XXXX", badge: "FREE", badgeColor: "#059669", helpText: "Free at developer.ebay.com", helpUrl: "https://developer.ebay.com", description: "Live eBay listing prices", categories: "All categories" },
  { id: "discogs", label: "Discogs Token", icon: "🎶", required: false, placeholder: "abCdEfGhIjKlMn...", badge: "FREE", badgeColor: "#059669", helpText: "Free at discogs.com/settings/developers", helpUrl: "https://www.discogs.com/settings/developers", description: "Marketplace prices for vinyl & CDs", categories: "Vinyl Records, CDs/DVDs" },
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", zIndex: 1000, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "16px", overflowY: "auto" }}>
      <div style={{ background: "#0a0a0a", border: "1px solid #d4a017", borderRadius: 16, padding: 24, width: "100%", maxWidth: 480, marginTop: 16 }}>
        ...
        <p style={{ color: "#525252", fontSize: 11, marginBottom: 20, marginTop: 0 }}>{connected} of {API_CONFIG.length} sources connected · Keys saved locally on this device only</p>
              ...
              <div key={cfg.id} style={{ background: hasKey ? "rgba(212,160,23,0.04)" : "#111", border: `1px solid ${hasKey ? "rgba(212,160,23,0.25)" : "#222"}`, borderRadius: 10, padding: 14 }}>
                ...
                <div style={{ position: "relative" }}>
                  <input
                    ...
                    value={vals[cfg.id] || ""}
                    onChange={(e) => setVals(v => ({ ...v, [cfg.id]: e.target.value }))}
        ...
        <div style={{ background: "rgba(212,160,23,0.04)", border: "1px solid rgba(212,160,23,0.15)", borderRadius: 8, padding: 10, margin: "20px 0" }}>
          <p style={{ fontSize: 10, color: "#525252", margin: 0 }}>🔒 Keys stored in your browser's localStorage only — never sent to any Trash or Treasure server.</p>
        </div>
...
export default function Home() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("tot_api_keys");
  ...
  const saveKeys = (keys) => {
    setApiKeys(keys);
    try { localStorage.setItem("tot_api_keys", JSON.stringify(keys)); } catch {}
  };
  ...
  const analyze = async () => {
    ...
    try {
      ...
      formData.append("ebayKey", apiKeys.ebay || "");
      formData.append("discogsKey", apiKeys.discogs || "");
      formData.append("justtcgKey", apiKeys.justtcg || "");
      const res = await fetch("/api/analyze", { method: "POST", body: formData });
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        if (errData.error === "NO_API_KEY" || errData.error === "INVALID_API_KEY") { setShowSettings(true); throw new Error("Please check your Gemini API key in Settings."); }
  ...
  const totalValue = results?.items?.reduce((sum, i) => sum + (i.recommended || i.estimatedValue || 0), 0) || 0;
    ...
    <main style={{ minHeight: "100vh", maxWidth: 480, margin: "0 auto", padding: "16px 16px 100px" }}>
      ...
      <header style={{ textAlign: "center", padding: "24px 0 16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginTop: 8 }}>
          ...
          <button onClick={() => setShowSettings(true)} style={{
            background: apiKeys.gemini ? "rgba(212,160,23,0.1)" : "rgba(220,38,38,0.1)",
            border: `1px solid ${apiKeys.gemini ? "rgba(212,160,23,0.3)" : "rgba(220,38,38,0.4)"}`,
            borderRadius: 6, padding: "4px 12px", fontSize: 11,
            color: apiKeys.gemini ? "#d4a017" : "#f87171",
      ...
      {!imagePreview ? (
        <section onDrop={handleDrop} onDragOver={(e) => e.preventDefault()}
          style={{ border: "2px dashed #333", borderRadius: 12, padding: "40px 20px", textAlign: "center", background: "var(--surface)", cursor: "pointer" }}
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
            <button onClick={(e) => { e.stopPropagation(); cameraInputRef.current?.click(); }}
              style={{ background: "linear-gradient(135deg, #d4a017, #b8860b)", color: "#000", border: "none", borderRadius: 8, padding: "10px 20px", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>
              📸 Take Photo
            </button>
            <button onClick={(e) => { e.stopPropagation(); fileInputRef.current?.click(); }}
              style={{ background: "var(--border)", color: "#e5e5e5", border: "none", borderRadius: 8, padding: "10px 20px", fontWeight: 600, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>
              📁 Upload
            </button>
          </div>
        ...
        <section>
          ...
          {!results && (
            <button onClick={analyze} disabled={loading}
              style={{ width: "100%", background: loading ? "#333" : "linear-gradient(135deg, #d4a017, #b8860b)", color: loading ? "#737373" : "#000", border: "none", borderRadius: 12, padding: "16px", fontSize: 16, fontWeight: 800, cursor: loading ? "wait" : "pointer", fontFamily: "inherit", letterSpacing: "0.5px", marginBottom: 16 }}>
      ...
      {error && (
        <div style={{ background: "rgba(220,38,38,0.1)", border: "1px solid rgba(220,38,38,0.3)", borderRadius: 8, padding: 12, marginTop: 12, color: "#fca5a5", fontSize: 13 }}>
          ⚠️ {error}
        </div>
      )}
      ...
      {results && (
        <section style={{ marginTop: 16 }}>
              ...
              <div key={i} style={{
                ...
                <div style={{ background: "#0a0a0a", borderRadius: 8, padding: "8px 10px" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 6, marginTop: 2 }}>
                    ...
                    <span style={{ fontSize: 18, fontWeight: 900, color: item.tier === "treasure" ? "#d4a017" : "#e5e5e5" }}>
                      ${(item.recommended || item.estimatedValue)?.toLocaleString() ?? "?"}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <p style={{ fontSize: 10, color: "#525252", textAlign: "center", marginTop: 16, lineHeight: 1.4 }}>
            Prices sourced from {selectedCat?.sources}. Actual prices vary by condition, edition and demand.
          </p>
          <button onClick={reset} style={{ width: "100%", background: "var(--surface)", color: "#d4a017", border: "1px solid #d4a017", borderRadius: 12, padding: "14px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", marginTop: 12 }}>
            📷 SCAN AGAIN
          </button>
        </section>
      )}
